...
def scheme_apply(procedure, args, env):
    ...
    def convert_pair(pair):
        if pair is nil:
            return []
        return [pair.first] + convert_pair(pair.rest)

    new_list = convert_pair(args)
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        if procedure.expect_env == True:
            new_list.append(env)
        try:
            return procedure.py_func(*new_list)
        ...
        # env = {x: 3, y: 7}
        # procedure.formals = (x)
        # still create new frame because whenever you call a function, you create new frame
